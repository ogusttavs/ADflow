...
function validateAudioUrl(audioUrl: string): string | null {
  let parsed: URL;
  try {
  ...
  if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
    return "A URL deve usar protocolo HTTP ou HTTPS";
  }

  const hostname = parsed.hostname.toLowerCase();
  if (!hostname) return "Host inválido";
  if (hostname === "localhost" || hostname.endsWith(".local")) {
    return "Host local não permitido";
  }
...
export async function transcribeAudio(
  ...
  try {
    ...
    if (urlValidationError) {
      return {
        error: "URL de áudio inválida",
        code: "INVALID_FORMAT",
        details: urlValidationError,
      };
    }
