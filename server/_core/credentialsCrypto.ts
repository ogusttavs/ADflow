...
function decodeEncryptionKey(rawKey: string): Buffer {
  ...
  if (key.byteLength !== 32) {
    throw new Error("CREDENTIAL_ENCRYPTION_KEY deve ter 32 bytes (hex ou base64)");
  }
...
function getEncryptionKey(): Buffer {
  ...
  cachedKey = decodeEncryptionKey(ENV.credentialEncryptionKey);
  return cachedKey;
}
...
export function decryptCredentialValue(encryptedValue: string): string {
...
export function decryptCredentialOrLegacy(value: string): string {
